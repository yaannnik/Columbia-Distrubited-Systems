...
const PASS_VALID_VALUES = "PASS_VALID_VALUES"
...
func valueIsValidString(values []string, s string) bool {
...
func valueIsValidInt(values []int, s int) bool {
	for _, v := range values {
    if v == s {
      return true
    }
...
func valueIsValid(values interface{}, s interface{}) bool {
	if v, ok := values.(string); ok && v == PASS_VALID_VALUES {
		return true
	}
	if s == nil {
		return false
	...
	if v, ok := s.(string); ok {
		if valid, ok := values.([]string); ok {
			return valueIsValidString(valid, v)
		}
	} else if v, ok := s.(int); ok {
		if valid, ok := values.([]int); ok {
			return valueIsValidInt(valid, v)
		}
	}
	return false
}

func ndecided(t *testing.T, pxa []*Paxos, seq int, values interface{}) int {
	count := 0
	var v interface{}
	for i := 0; i < len(pxa); i++ {
		if pxa[i] != nil {
			...
			if decided {
				...
				if !valueIsValid(values, v1) {
					t.Fatalf("decided value is invalid; seq=%v i=%v value=%v valid=%v",
						seq, i, v1, values)
				}
				if count > 0 && v != v1 {
...
func waitn(t *testing.T, pxa []*Paxos, seq int, wanted int, values interface{}) {
	to := 10 * time.Millisecond
	for iters := 0; iters < 30; iters++ {
		if ndecided(t, pxa, seq, values) >= wanted {
			break
		}
		time.Sleep(to)
		if to < time.Second {
	...
	nd := ndecided(t, pxa, seq, values)
	if nd < wanted {
		t.Fatalf("too few decided; seq=%v ndecided=%v wanted=%v", seq, nd, wanted)
	}
}

func waitmajority(t *testing.T, pxa []*Paxos, seq int, values interface{}) {
	waitn(t, pxa, seq, (len(pxa)/2)+1, values)
}

func checkmax(t *testing.T, pxa []*Paxos, seq int, max int, values interface{}) {
...
func TestBasic(t *testing.T) {
	...
	pxa[0].Start(0, "hello")
	waitn(t, pxa, 0, npaxos, []string{"hello"})
	...
	pxa[1].Start(2, 101)
	pxa[2].Start(2, 102)
	waitn(t, pxa, 2, npaxos, []int{100, 101, 102})
	...
	pxa[0].Start(6, 600)
	pxa[1].Start(5, 500)
	waitn(t, pxa, 7, npaxos, []int{700, 600, 500})
	pxa[0].Start(4, 400)
	pxa[1].Start(3, 300)
	waitn(t, pxa, 6, npaxos, []int{700, 600, 500})
	waitn(t, pxa, 5, npaxos, []int{700, 600, 500})
	waitn(t, pxa, 4, npaxos, []int{400, 300})
	waitn(t, pxa, 3, npaxos, []int{400, 300})
...
func TestDeaf(t *testing.T) {
	...
	waitmajority(t, pxa, 1, []string{"goodbye"})
	time.Sleep(1 * time.Second)
	if ndecided(t, pxa, 1, []string{"goodbye"}) != npaxos-2 {
		t.Fatalf("a deaf peer heard about a decision")
	}

	pxa[0].Start(1, "xxx")
	waitn(t, pxa, 1, npaxos-2, []string{"goodbye"})
	time.Sleep(1 * time.Second)
	if ndecided(t, pxa, 1, []string{"goodbye"}) != npaxos-1 {
		t.Fatalf("a deaf peer heard about a decision")
	}

	pxa[npaxos-1].Start(1, "yyy")
	waitn(t, pxa, 1, npaxos, []string{"goodbye"})
...
func TestForget(t *testing.T) {
	...
	waitn(t, pxa, 0, npaxos, []string{"00", "11", "22", "66", "77"})
	...
	waitn(t, pxa, 1, npaxos, []string{"00", "11", "22", "66", "77"})
...
func TestForgetMem(t *testing.T) {
	...
	for i := 1; i <= 10; i++ {
		...
		waitn(t, pxa, i, npaxos, []string{string(big)})
	}
...
func TestRPCCount(t *testing.T) {
	...
	for i := 0; i < ninst1; i++ {
		pxa[0].Start(seq, "x")
		waitn(t, pxa, seq, npaxos, []string{"x"})
		seq++
	}
	...
	for i := 0; i < ninst2; i++ {
		for j := 0; j < npaxos; j++ {
			values = append(values, j+(i*10))
			go pxa[j].Start(seq, j+(i*10))
		}
		waitn(t, pxa, seq, npaxos, values)
		seq++
	}
...
func TestMany(t *testing.T) {
	...
	for seq := 1; seq < ninst; seq++ {
		...
		for seq >= 5 && ndecided(t, pxa, seq-5, values) < npaxos {
			time.Sleep(20 * time.Millisecond)
		}
		for i := 0; i < npaxos; i++ {
			values = append(values, (seq*10)+i)
	...
	for {
		...
		for seq := 1; seq < ninst; seq++ {
			if ndecided(t, pxa, seq, values) < npaxos {
				done = false
			}
		}
		if done {
...
func TestOld(t *testing.T) {
	...
	waitmajority(t, pxa, 1, []int{111})

	pxa[0] = Make(pxh, 0, nil)
	pxa[0].Start(1, 222)

	waitn(t, pxa, 1, 4, []int{111})

	if false {
		pxa[4] = Make(pxh, 4, nil)
		waitn(t, pxa, 1, npaxos, []int{111})
	}
...
func TestManyUnreliable(t *testing.T) {
	...
	for seq := 1; seq < ninst; seq++ {
		...
		// number of file descriptors.
		for seq >= 3 && ndecided(t, pxa, seq-3, values) < npaxos {
			time.Sleep(20 * time.Millisecond)
		}
		for i := 0; i < npaxos; i++ {
			values = append(values, (seq*10)+i)
...
func TestPartition(t *testing.T) {
	...
	time.Sleep(2 * time.Second)
	waitmajority(t, pxa, seq, []int{111})
	...
	waitn(t, pxa, seq, npaxos, []int{111})
	...
	for iters := 0; iters < 20; iters++ {
		...
		waitmajority(t, pxa, seq, []int{seq * 10})
		if ndecided(t, pxa, seq, []int{seq * 10}) > 3 {
			t.Fatalf("too many decided")
		}

		part(t, tag, npaxos, []int{0, 1}, []int{2, 3, 4}, []int{})
		waitn(t, pxa, seq, npaxos, []int{seq * 10})
	}
	...
	for iters := 0; iters < 20; iters++ {
		...
		waitn(t, pxa, seq, 3, values)
		if ndecided(t, pxa, seq, values) > 3 {
			t.Fatalf("too many decided")
		}
...
func TestLots(t *testing.T) {
	...
	go func() {
		...
		for done == false {
			...
			nd := 0
			for i := 0; i < seq; i++ {
				if ndecided(t, pxa, i, PASS_VALID_VALUES) == npaxos {
					nd++
				}
			}
			if seq-nd < 10 {
	...
	go func() {
		...
		for done == false {
			for i := 0; i < seq; i++ {
				ndecided(t, pxa, i, PASS_VALID_VALUES)
			}
			time.Sleep(time.Duration(rand.Int63()%300) * time.Millisecond)
		}
	}()
	...
	for i := 0; i < seq; i++ {
		waitmajority(t, pxa, i, PASS_VALID_VALUES)
	}
