...
// Node An interface to represent a node in a distributed system.
...
type Node interface {
	...
	// MessageHandler Why a node may return multiple new nodes? Because a node could make an decision during
	...
	// NextTimer Get the next timer that will be triggered in this node
	NextTimer() Timer

	// TriggerTimer Trigger the next timer and return a list of new nodes. Check `MessageHandler` for why
	// we need to return a list of nodes.
	TriggerTimer() []Node

	// HandlerResponse When a node handles an event (message + timer), it will return a list of new nodes.
	...
	// Attribute Return the attribute of a node.
	Attribute() interface{}
...
// CoreNode A helper type to help users to implement some functions of Node interface.
